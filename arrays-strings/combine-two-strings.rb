...
def valid_shuffle?(str1, str2, str3)
  str3.each_char do |char|
    return false if str1.empty? && str2.empty?
    if str1.start_with?(char)
      str1[0] = ""
    elsif str2.start_with?(char)
      str2[0] = ""
    else
      return false
    end
...
p valid_shuffle?("abc", "def", "dabcef") #== true
p valid_shuffle?("abc", "def", "abdefc") #== true
p valid_shuffle?("abc", "def", "abfdec") #== false